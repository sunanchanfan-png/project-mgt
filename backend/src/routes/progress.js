...
router.get('/weekly', async (req, res) => {
  try {
    ...
    const withProgress = allActivities
      ...
      // ยกเว้น 2 กรณี: (1) ตอนเรียกจาก Tab9 "เล่มรายงาน" (ส่ง include_completed=true มา) ซึ่งอยากเห็นงานที่
      // เพิ่งเสร็จในสัปดาห์นั้นโชว์อยู่ในรายงานด้วย (2) สัปดาห์ที่กำลังดูอยู่ "ยังไม่จบจริง" (วันนี้ยังไม่ถึง
      // วันสิ้นสุดสัปดาห์) — เพราะระหว่างสัปดาห์ยังไม่จบ อาจมีการแก้ % ลดลงจาก 100% กลับไปได้อีก (พิมพ์ผิด/
      // กลับใจ) ถ้าตัดออกจากตารางทันทีที่ครบ 100% จะไม่มีทางกลับมาแก้ไขงานนั้นได้อีกจนกว่าจะไปหาทางอื่น (เช่น
      // ช่องค้นหาที่เอาออกไปแล้ว) — คงไว้ให้เห็น/แก้ไขได้ตลอดจนกว่าจะข้ามไปสัปดาห์ใหม่ ถึงจะกลับไปใช้ logic
      // เดิม (ซ่อนงานเสร็จแล้ว) ตามปกติ ไม่ต้องพึ่ง as_of/week_number เพราะจุดนี้เทียบกับ "วันนี้จริง" เสมอ
      // (ไม่ใช่ report.week_end) ถูกต้องอยู่แล้วเพราะ "จบสัปดาห์" ต้องอิงเวลาจริง ไม่อิงว่าใครเลือกดูสัปดาห์ไหน
